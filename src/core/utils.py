...
def load_config(config):
    if config['tiny']:
        ...
        ANCHORS = get_anchors(config['YOLO']['ANCHORS_TINY'], config['tiny'])
        XYSCALE = config['YOLO']['XYSCALE_TINY'] if config['model_type'] == 'yolov4' else [1, 1]
    else:
        STRIDES = np.array(config['YOLO']['STRIDES'])
        if config['model_type'] == 'yolov4':
            ANCHORS = get_anchors(config['YOLO']['ANCHORS'], config['tiny'])
        elif config['model_type'] == 'yolov3':
            ANCHORS = get_anchors(config['YOLO']['ANCHORS_V3'], config['tiny'])
        XYSCALE = config['YOLO']['XYSCALE'] if config['model_type'] == 'yolov4' else [1, 1, 1]
    NUM_CLASS = len(config['YOLO']['CLASSES'])
